...
# -*- coding: utf-8 -*-
"""
Created on Mon Jun 29 09:16:56 2020
...
#      entries = range(1024000) # 1 million entries
        ...
        background = ls[0]
        current = ls[1]
        stage1 = OrderedStage(increment, ls_shape[0]*ls_shape[1])
        pipe = Pipeline(stage1)
        for i in range(ls_shape[0]):
            for j in range(ls_shape[1]):
                pipe.put(background[i][j], current[i][j])
